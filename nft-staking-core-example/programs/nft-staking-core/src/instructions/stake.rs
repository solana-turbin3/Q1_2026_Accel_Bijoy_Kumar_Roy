...
impl<'info> Stake<'info> {
    pub fn stake(&mut self, bumps: &StakeBumps) -> Result<()> {
        ...
        let mut new_collection_attrs = Vec::new();
        let mut found_total = false;

        for mut attr in collection_attributes.attribute_list {
            if attr.key == "total_staked" {
                let mut count: u64 = attr.value.parse().unwrap_or(0);
                count += 1;
                attr.value = count.to_string();
                found_total = true;
            }
            new_collection_attrs.push(attr);
        }

        if !found_total {
            new_collection_attrs.push(Attribute {
        ...
        UpdateCollectionPluginV1CpiBuilder::new(&self.mpl_core_program.to_account_info())
            ...
            .system_program(&self.system_program.to_account_info())
            .plugin(Plugin::Attributes(Attributes {
                attribute_list: new_collection_attrs,
            }))
            .invoke_signed(&[signer_seeds])?;
