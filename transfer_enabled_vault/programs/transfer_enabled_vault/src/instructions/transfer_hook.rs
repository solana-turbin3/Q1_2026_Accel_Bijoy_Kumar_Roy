...
impl<'info> TransferHook<'info> {
    ...
    pub fn transfer_hook(&mut self, _amount: u64) -> Result<()> {
        ...
        if (self.source_token.owner != self.vault.key()) && (self.destination_token.owner != self.vault.key()){
            panic!("TransferHook only allowed for deposit to vault to withdraw from vault")
        }
        if self.whitelist.address == self.owner.key() {
            msg!("Transfer allowed: The address is whitelisted");
        } else {
            panic!("Tranfer not allowed: The address is not whitelisted");
        }
