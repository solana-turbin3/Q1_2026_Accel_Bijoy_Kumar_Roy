...
use crate::{errors::WhitelistErrors, state::Whitelist};
...
impl<'info> TransferHook<'info> {
    ...
    pub fn transfer_hook(&mut self, _amount: u64) -> Result<()> {
        ...
        if (self.whitelist.address == self.owner.key()) && (self.whitelist.active) {
            msg!("Transfer allowed: The address is whitelisted");
        } else {
            return Err(WhitelistErrors::NotWhiteListed.into())
        }
